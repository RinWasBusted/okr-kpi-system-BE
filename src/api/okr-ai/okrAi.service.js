...
function buildPrompt({ objective, cycle, unit, existingKeyResults, visibleObjectives, input, owner, parentObjective }) {
  ...
  return [
    ...
    `- Evaluate each suggested Key Result: provide a 'fit_score' (integer 0-100) indicating how strongly it aligns with the Objective, a 'fit_reason' explaining the score, and a list of any potential 'issues'.`,
    contextInstruction,
    ``,
    `JSON shape:`,
    `{"suggestions":[{"title": "...","target_value": 0,"start_value": 0,"unit":"...","weight": 25,"due_date":"YYYY-MM-DD","evaluation_method":"MAXIMIZE|MINIMIZE|TARGET","evaluation":{"fit_score": 85,"fit_reason":"...","issues":["..."]}}],"overall_feedback":{"summary":"...","alignment_analysis":"...","risks":["..."],"recommendations":["..."]}}`,
    ``,
    `Context JSON:`,
...
export async function generateKeyResultsForObjective({ objectiveId, user, input }) {
  ...
  try {
    ...
    const suggestions = normalizeWeights(parsed.suggestions, remainingWeight).map((s) => ({
      title: s.title,
      target_value: Math.round((s.target_value || 0) * 100) / 100,
      start_value: Math.round((s.start_value || 0) * 100) / 100,
      unit: s.unit,
      weight: s.weight,
      due_date: s.due_date,
