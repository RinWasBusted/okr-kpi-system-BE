...
export const createKPIRecord = async (user, assignmentId, payload) => {
  ...
  if (hasChildren) {
    throw new AppError(
      "Cannot create KPI records for assignment with children",
      400,
    );
  }
  ...
  if (!canCreate) {
    if (assignment.unit_id) {
      ...
      const isFromAncestorUnit =
        user.unit_id &&
        ownerUnit?.unit_id &&
        user.unit_id !== ownerUnit.unit_id &&
        (await isAncestorUnit(user.unit_id, ownerUnit.unit_id));
  ...
  if (!canCreate) {
    throw new AppError(
      "You do not have permission to create records for this assignment",
      403,
    );
  }
  ...
  const elapsedDays = Math.max(
    0,
    Math.min(totalPeriodDays, (now - periodStart) / (1000 * 60 * 60 * 24)),
  );
  const timeElapsedPercentage =
    totalPeriodDays > 0 ? (elapsedDays / totalPeriodDays) * 100 : 0;
  ...
  try {
    await notifyKPIAssignmentEvent({
      ...
      actorId: user.id,
      extraContext: `Tiến độ mới: ${Math.round(record.progress_percentage * 100) / 100}%`,
    });
  } catch (error) {
...
export const listKPIRecords = async (user, assignmentId) => {
  ...
  if (!canView) {
    throw new AppError(
      "You do not have permission to view records for this assignment",
      403,
    );
  }
...
const canViewAssignment = async (user, assignment) => {
  ...
  if (assignment.visibility === "PRIVATE") {
    ...
    return (
      userUnit !== assignmentUnit &&
      (await isAncestorUnit(userUnit, assignmentUnit))
    );
  }
...
export const getKPIChartData = async (user, unitId) => {
  ...
  if (user.role !== "ADMIN_COMPANY") {
    ...
    if (!userPath) {
      throw new AppError(
        "You do not have permission to view this unit's data",
        403,
      );
    }
  ...
  const assignmentToKpiMap = new Map(
    assignments.map((a) => [a.id, a.kpi_dictionary.id]),
  );
