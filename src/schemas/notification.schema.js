...
export const createNotificationSchema = z.object({
    ...
    refType: z.string().min(1, "Reference type is required").trim(),
    refId: z.number().int().positive("Reference ID must be a positive integer"),
    actorId: z
        .number()
        .int()
    ...
    extraContext: z
        .string()
        .max(150, "Extra context cannot exceed 150 characters")
        .optional(),
    recipientIds: z
        .array(
            z.number().int().positive("Each recipient ID must be a positive integer"),
        )
